...
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoModelForSequenceClassification, DataCollatorForLanguageModeling
...
def main(
    ...
    model = AutoModelForSequenceClassification.from_pretrained(reward_model_id, revision=model_revision, **build_model_types(dtype, device)).eval()
    ...
    if getattr(tokenizer, "chat_template"):
        tokenizer.chat_template = PROMPT_TEMPLATES[reward_prompt_template or reward_model_id]
    ...
    with jsonlines.open(reward_output_filename, "a") as fout:
        ...
        for i in range(0, len(dataset), batch_size):
            ...
            for j in range(i, end_i):
                item = dataset[j]
                conv = item["conversations"] + [{'role': 'assistant', 'content': item['response']}]
